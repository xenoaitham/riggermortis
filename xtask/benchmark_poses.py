...
- ``usable-straight-away`` = solver says reliable AND every distal flip is
  verified AND no deep-foreshortening flag. Flip verification semantics
  (D-010): 1.0 = limb straight within ~14.5 deg (immaterial, auto-pass) or
  bend resolved confidently by the priors; 0.0 = distal joint unobserved
  (flip unverifiable — the honest failure; S4's "unmeasured flips" were this
  plus straight-limb ties reading as zero margins, fixed 2026-09-15). The
  metric is a *lower bound* on "artist can use this pose immediately"; it
  does not measure anatomical correctness of the depth solve (the D-008
  kick / arms-back classes are covered by the foreshortening flag, not by
  flip margins).
- ``flip_uncertain`` column = flips needing human review (unobserved distal
  joints); ``deep_foreshortening`` = proximal segment 2D length well below
  its canonical 3D length (>=0.6 cut) — flagged as a heuristic, not a verdict.
...
def run_image(engine, path: Path) -> Row | None:
    _core, dwpose, FigureBoard, observations_from_keypoints, solve_pose, conf_bar = engine
...
def _fmt_stats(rows: list[Row]) -> str:
    ...
    no_person = sum(1 for r in rows if r.figures == 0)
    tail = f"; detector found no person: {no_person}/{n}" if no_person else ""
    return (
...
def markdown_block(sets: dict[str, list[Row]]) -> str:
    lines = [
        ...
        "",
        (
            "`usable-straight-away` = solver-reliable AND every distal flip verified "
            "(>= 0.55; straight limbs are immaterial and auto-pass, unobserved "
            "distal joints read 0.0 = review, D-010) AND no deep-foreshortening "
            "flag (heuristic proxy for the two documented D-008 miss classes: "
            "deep kicks, wrists behind the back). Flags: `flip` = distal joint "
            "unseen, bend unverifiable; `fore-<segment>` = proximal segment 2D "
            "length < 0.6x canonical (single-view depth limitation)."
        ),
        "",
    ]
